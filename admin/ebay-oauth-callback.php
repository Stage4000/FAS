...
// Get the access token for immediate use and refresh token for long-term storage
$accessToken = $tokenData['access_token'];
$refreshToken = $tokenData['refresh_token'] ?? null;

// For Inventory API, we use the access token
// Note: Access tokens expire after 2 hours, so we should also save the refresh token
// to get new access tokens when needed
...
// Also save the refresh token for future token renewal
if ($refreshToken) {
    $config['ebay']['refresh_token'] = $refreshToken;
}

// Save token expiry information
$config['ebay']['token_expires_at'] = time() + ($tokenData['expires_in'] ?? 7200);
...
if (file_put_contents($configFile, $configContent)) {
    ...
    $_SESSION['success'] = 'eBay OAuth token obtained successfully! Access token is valid for 2 hours. Use the refresh token for renewal.';
} else {
